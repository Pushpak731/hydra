...
def _hydra_cause_wrapper(error: BaseException) -> bool:
    ...
    if not instantiation_wrapper and error_type is not ImportError:
        return False
    ...
    return (instantiation_wrapper and _instantiation_frame(tb)) or (
        error_type is ImportError and _traceback_module(tb) == "hydra._internal._locate"
    )
...
def _report_instantiation_exception(ex: InstantiationException) -> None:
    ...
    try:
        ...
        while pending:
            ...
            saved_args.append((current, current.args))
        ...
        for error, original_tb in saved_tracebacks:
            error.__traceback__ = original_tb
